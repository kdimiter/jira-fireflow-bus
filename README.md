...
```sh
# 1. Download these two assets from release v0.3.7, then verify and run:
...
Use a unique password generated by your password manager. `prepare-jira.sh` creates or discovers the company-managed
project, the selected standard work type, three result fields, and dedicated screen schemes. Install
...
Download: [GitHub Release v0.3.7](https://github.com/kdimiter/jira-fireflow-bus/releases/tag/v0.3.7).
...
python3 scripts/build-installer.py \
  --output dist/algosec-jira-bus-0.3.7-linux.run
sh packaging/docker/build-image.sh \
  dist/algosec-jira-bus-0.3.7-linux.run \
  dist/algosec-jira-bus-0.3.7-docker-amd64.tar.gz
python3 scripts/build-docker-installer.py \
  --image dist/algosec-jira-bus-0.3.7-docker-amd64.tar.gz \
  --output dist/algosec-jira-bus-0.3.7-docker-amd64.run
python3 scripts/build-release-metadata.py \
  --directory dist --version 0.3.7 --image algosec-jira-bus:0.3.7
```
