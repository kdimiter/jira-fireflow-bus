...
```sh
# 1. Download these two assets from release v0.2.3, then verify and run:
...
Download: [GitHub Release v0.2.3](https://github.com/kdimiter/jira-fireflow-bus/releases/tag/v0.2.3).
...
python3 scripts/build-installer.py \
  --output dist/algosec-jira-bus-0.2.3-linux.run
sh packaging/docker/build-image.sh \
  dist/algosec-jira-bus-0.2.3-linux.run \
...
python3 scripts/build-docker-installer.py \
  ...
  --output dist/algosec-jira-bus-0.2.3-docker-amd64.run
python3 scripts/build-release-metadata.py \
  --directory dist --version 0.2.3 --image algosec-jira-bus:0.2.3
```
