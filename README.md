...
the selected non-root operator; deploys or reuses one Forge App; prepares the chosen Jira
Space and work type; and finally validates and starts the Docker bus. Forge credentials are
used only during deployment. The runtime Jira and FireFlow secrets remain in the private
container configuration managed by the existing wizard.
...
project, the selected standard work type, three result fields, and dedicated screen and field-configuration schemes. The Forge field is required; FireFlow ID, status and owner are visible but optional. Install
...
python3 scripts/build-installer.py \
  --output dist/algosec-jira-bus-0.3.8-linux.run
sh packaging/docker/build-image.sh \
  dist/algosec-jira-bus-0.3.8-linux.run \
  dist/algosec-jira-bus-0.3.8-docker-amd64.tar.gz
python3 scripts/build-docker-installer.py \
  --image dist/algosec-jira-bus-0.3.8-docker-amd64.tar.gz \
