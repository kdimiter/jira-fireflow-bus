...
```sh
# 1. Download these two assets from release v0.3.6, then verify and run:
...
Download: [GitHub Release v0.3.6](https://github.com/kdimiter/jira-fireflow-bus/releases/tag/v0.3.6).
...
python3 scripts/build-installer.py \
  --output dist/algosec-jira-bus-0.3.6-linux.run
sh packaging/docker/build-image.sh \
  dist/algosec-jira-bus-0.3.6-linux.run \
  dist/algosec-jira-bus-0.3.6-docker-amd64.tar.gz
python3 scripts/build-docker-installer.py \
  --image dist/algosec-jira-bus-0.3.6-docker-amd64.tar.gz \
  --output dist/algosec-jira-bus-0.3.6-docker-amd64.run
python3 scripts/build-release-metadata.py \
  --directory dist --version 0.3.6 --image algosec-jira-bus:0.3.6
```
