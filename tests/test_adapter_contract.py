...
class AdapterContract(unittest.TestCase):
    def check_refused(self, issue_value, mapping):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn('adapter', result['refused'][0][1].lower())
            self.assertEqual(f.calls, [])

    def test_named_objects_are_refused_if_installed_adapter_only_accepts_addresses(self):
        self.check_refused(issue(source='srv-app-01'), MAPPING)
    ...
    def test_101_lines_are_refused_by_current_adapter_before_apply(self):
        ...
        self.check_refused(table_issue(document(table(grid))), TABLE_MAPPING)
