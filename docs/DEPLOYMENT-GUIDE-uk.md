...
```sh
git clone --branch v0.3.4 --depth 1 https://github.com/kdimiter/jira-fireflow-bus.git
...
Назва першого результатного поля — саме **FireFlow Request ID**, а не скорочене «FireFlow ID». FireFlow Owner залишається окремим текстовим полем, а системний Jira Assignee може додатково синхронізуватися через явну таблицю відповідностей. Status є текстом, не списком статусів workflow. У company-managed Jira точний API-тип усіх трьох результатних полів — `com.atlassian.jira.plugin.system.customfieldtypes:textfield`; саме його створює `prepare-jira.sh`. Уже наявні поля правильного типу використовуйте повторно.
...
Відкрийте [GitHub Release v0.3.4](https://github.com/kdimiter/jira-fireflow-bus/releases/tag/v0.3.4)
і завантажте два assets:

- `algosec-jira-bus-0.3.4-docker-amd64.run`
- `algosec-jira-bus-0.3.4-docker-amd64.run.sha256`
...
Щоб Jira **Assignee** повторював FireFlow **Owner**, після запуску контейнера виконайте:

```sh
sudo bus_conf --owner-assignees
```

Для кожного FireFlow owner введіть його точне ім'я, потім email або display name користувача
Jira і виберіть знайдений запис. Майстер показує лише користувачів, яких Jira дозволяє
призначати у налаштованому Space, та зберігає стабільний `accountId`. Порожнє ім'я завершує
налаштування; запис `-owner_name` видаляє відповідність. Якщо власник не має Jira-акаунта,
не додавайте відповідність: його ім'я залишиться у полі FireFlow Owner, а поточний Assignee
не буде підмінено. Зміна таблиці не перепитує і не перезаписує API credentials або state.

Для кожної нової заявки шина передає `creator.emailAddress` із Jira у поле FireFlow
**Requestor**. Поле **Owner** шина не задає: його призначає workflow FireFlow. Якщо Jira API
не повертає email автора через налаштування приватності, заявка не надсилається з хибним
requestor; у журналі буде чітка помилка про недоступний `Jira creator email`.
...
З [GitHub Release v0.3.4](https://github.com/kdimiter/jira-fireflow-bus/releases/tag/v0.3.4)
завантажте `algosec-jira-bus-0.3.4-linux.run` і сусідній `.sha256`, потім:

```sh
sha256sum -c algosec-jira-bus-0.3.4-linux.run.sha256
sudo sh algosec-jira-bus-0.3.4-linux.run
