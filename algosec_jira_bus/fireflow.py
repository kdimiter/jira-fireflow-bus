...
Only authenticate, list templates, read a traffic request and create a traffic request
are exposed.  A create intent is durably claimed before its POST, so an interrupted call
cannot be retried as though nothing happened.
"""
import hashlib
import json
import os
...
from .transport import MAX_REQUEST_BYTES, https_origin, request_json
...
class FireFlow:
    def __init__(self, config, mode, audit, request=None, resolver=None):
