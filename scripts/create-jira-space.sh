#!/bin/sh
# Create or verify a Jira Space through the installed Docker image; no host Python is used.
set -eu
PATH=/usr/sbin:/usr/bin:/sbin:/bin
export PATH
unset PYTHONPATH PYTHONHOME DOCKER_HOST DOCKER_CONTEXT DOCKER_CONFIG
CONTAINER=algosec-jira-bus
IMAGE=algosec-jira-bus:0.3.5
...
exec docker run --rm "$TTY" --read-only --tmpfs /tmp:rw,nosuid,nodev,size=32m \
    ...
    -m algosec_jira_bus.jira_provision --space-only "$@"
